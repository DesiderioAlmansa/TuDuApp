...
export default function NavMenu({userName} : NavMenuProps) {
  ...
  const logout = () => {
    localStorage.removeItem('AUTH_TOKEN_TUDUAPP')
    queryClient.invalidateQueries({queryKey: ['user']})
  }

  return (
    <Popover className="relative">
      <Popover.Button className="inline-flex items-center gap-x-1 text-sm font-semibold leading-6 p-1 rounded-lg bg-purple-800">
